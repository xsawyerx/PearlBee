...
our @ISA = qw(Exporter);
our @EXPORT_OK
    = qw/get_total_pages get_previous_next_link generate_pagination_numbering/;
...
sub get_total_pages {
    ...
    my $total_pages
        = (
        ( $nr_of_items / $nr_of_rows ) != int( $nr_of_items / $nr_of_rows ) )
        ? int( $nr_of_items / $nr_of_rows ) + 1
        : ( $nr_of_items / $nr_of_rows );
...
sub generate_pagination_numbering {
    my ( $total_entries, $entries_per_page, $current_page, $pages_per_set )
        = @_;
